...
module.exports = async (ctx) => {
    ...
    const siteConfig = {
        kemono: {
            domain: 'pawchive.pw',
            imgDomain: 'img.pawchive.pw',
            fileDomain: 'file.pawchive.pw',
            name: 'Kemono',
        },
        coomer: {
            domain: 'coomer.st',
            imgDomain: 'coomer.st',
            fileDomain: 'coomer.st',
            name: 'Coomer',
        },
    };
    ...
    const items = postsData.map((i) => {
        let description = i.content || i.substring || '';

        if (i.file && i.file.path) {
            const fileUrl = site === 'kemono' ? `https://${config.imgDomain}/thumbnail/data${i.file.path}` : `https://${config.domain}/data${i.file.path}`;
        ...
        if (i.attachments && i.attachments.length > 0) {
            for (const attachment of i.attachments) {
                let attachmentUrl;
                if (site === 'kemono') {
                    attachmentUrl = /\.(jpg|jpeg|png|gif|webp)$/i.test(attachment.name)
                        ? `https://${config.imgDomain}/thumbnail/data${attachment.path}`
                        : `https://${config.fileDomain}/data${attachment.path}?f=${encodeURIComponent(attachment.name)}`;
                } else {
                    attachmentUrl = `https://${config.domain}/data${attachment.path}`;
                }
